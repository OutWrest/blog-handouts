# original source: https://github.com/mlomnitz/DiffJPEG
...
class y_quantize(nn.Module):
    ...
    Output:
        image(tensor): batch x height x width
    """
    def __init__(self, rounding, device, factor=1):
        super(y_quantize, self).__init__()
        self.rounding = rounding
        self.factor = factor
        self.y_table = nn.Parameter(torch.from_numpy(utils.y_table).to(device))
...
class c_quantize(nn.Module):
    ...
    def __init__(self, rounding, device, factor=1):
        ...
        self.rounding = rounding
        self.factor = factor
        self.c_table = nn.Parameter(torch.from_numpy(utils.c_table).to(device))

    def forward(self, image):
        if self.c_table.device != image.device:
            self.c_table = self.c_table.to(image.device)
...
class compress_jpeg(nn.Module):
    ...
    def __init__(self, device, rounding=torch.round, factor=1):
        ...
        self.c_quantize = c_quantize(rounding=rounding, device=device, factor=factor)
        self.y_quantize = y_quantize(rounding=rounding, device=device, factor=factor)
